import React, {useState} from "react";
import { AiOutlineMenu, AiOutlineSearch, AiOutlineClose, AiFillTag} from "react-icons/ai";
import {BsFillCartFill, BsFillSaveFill} from 'react-icons/bs'
import {TbTruckDelivery} from 'react-icons/tb'
import {FaUserFriends, FaWallet} from 'react-icons/fa'
import {MdHelp, MdFavorite} from 'react-icons/md'
const Navbar = () => {
    const [nav, setNav] = useState(false)
  return (
    <div className="max-w-[1640px] mx-auto flex justify-between items-center p-4">
      {/* Left side of NavBar - start */}
      <div className="flex items-center">
        <div onClick={()=> setNav(!nav)} className="cursor-pointer">
          <AiOutlineMenu size={30} color={"black"} />
        </div>
        <div className="flex">
          <h1 className="text-2xl sm:text-3xl md:text-4xl px-2">
            Just <span className="font-bold">Eats</span>
          </h1>
          <div className="hidden sm:flex items-center bg-gray-200 rounded-full p-1 text-[14px] justify-between">
            <p className="bg-black text-white rounded-full p-2">Delivery</p>
            <p className="p-2">Pickup</p>
          </div>
        </div>
      </div>
      {/* Left side of NavBar - End */}
      {/* Search Bar - Start */}
      <div className="bg-gray-200 rounded-full flex items-center px-2 w-[200px] sm:w-[400px] lg:w-[500px]">
        <AiOutlineSearch size={20}/>
        <input className="bg-transparent p-2 w-full focus:outline-none" type="text" placeholder="Search Foods"/>
      </div>
      {/* Search Bar - End */}
      {/* Cart Button - Start */}
      <button className="bg-black rounded-full text-white hidden sm:flex py-2 items-center">
        <BsFillCartFill size={20} className="mr-2"/>
      </button>
      {/* Cart Button - End */}
      {/* Overlay - Start */}
      {nav ? <div className="bg-black/80 fixed w-full h-screen z-10 top-0 left-0"></div> : ''}
      {/* Overlay - End */}
      {/* side drawer - Start */}
      <div className={nav ? "fixed top-0 left-0 w-[300px] h-screen z-10 bg-white duration-300" : "fixed top-0 left-[-100%] w-[300px] h-screen z-10 bg-white duration-300"}>
        <AiOutlineClose onClick={()=>setNav(!nav)} size={30} className="absolute right-4 top-4 cursor-pointer" />
        <h2 className="text-2xl p-4">Just <span className="font-bold">Eats</span></h2>
        <nav>
        <ul className="flex flex-col p-4 text-gray-800">
            <li className="text-xl py-4 flex">
                <TbTruckDelivery size={30} className="mr-4"/>
                Orders
            </li>
            <li className="text-xl py-4 flex">
                <MdFavorite size={30} className="mr-4"/>
                Favorites
            </li>
            <li className="text-xl py-4 flex">
                <FaWallet size={30} className="mr-4"/>
                Wallet
            </li>
            <li className="text-xl py-4 flex">
                <MdHelp size={30} className="mr-4"/>
                Help
            </li>
            <li className="text-xl py-4 flex">
                <AiFillTag size={30} className="mr-4"/>
                Promotions
            </li>
            <li className="text-xl py-4 flex">
                <BsFillSaveFill size={30} className="mr-4"/>
                Best one
            </li>
            <li className="text-xl py-4 flex">
                <FaUserFriends size={30} className="mr-4"/>
                Invite Friends
            </li>
        </ul>
      </nav>
      </div>
      {/* side drawer - End */}
    </div>
  );
};

export default Navbar;
